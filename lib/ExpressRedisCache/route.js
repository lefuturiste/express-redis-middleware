  ...
  return function expressRedisCache_Middleware(req, res, next) {
    ...
    self.get(name, (error, cache) => {
      ...
      if (cache.length && cache[0].body != null) {
        ...
        let status = res.status.bind(res);
        ...
        res.send = function(body) {
          ...
          /** limit the caching to a small subset of http status **/
          const allowedStatus = [200, 202, 203, 208, 226]
          if (!allowedStatus.includes(returnedStatus)) {
            return ret
          }
